...
**Asset Palette Loading**: `NGEngineInit()` automatically calls `NGPalInitAssets()` if your project includes `progear_assets.h`. This uses weak symbol linking - the generated function overrides the SDK's no-op default.
...
High-level sprite management with animation support. Actor graphics are defined in `assets.yaml` and processed by `progear_assets.py`.

```c
// Create actor (NGVisualAsset_xxx defined in progear_assets.h)
...
Run `python3 tools/progear_assets.py assets.yaml -o build/` to generate assets.
