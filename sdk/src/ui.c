...
static void render_panel_9slice(NGMenu *menu, s16 screen_x, s16 screen_y) {
    ...
    /* SCB1: Write tile data for each column.
     * SCB1 entries are 64 words apart per sprite, so we must set address per column.
     * However, we only set VRAMMOD once since it persists. */
    NG_REG_VRAMMOD = 1;
    ...
    /* SCB2: Write shrink values with auto-increment */
    ...
    /* SCB3: Write Y position and height with auto-increment */
    ...
    /* SCB4: Write X positions with auto-increment */
